/**
* @autor Tolga Zülfikaroglu
* Hier wird das Textresultat (final_transcript) bzw. der Audio-Stream, der in einen Text umgewandelt wurde, tokenisiert.
* (uncommonArr): in diesem Array werden die wichtigen Tokens gespeichert.
*/
...
* Tokenizer Funktion
* Hier wird das Textresultat (final_transcript) bzw. der Audio-Stream, der in einen Text umgewandelt wurde, tokenisiert.
* (stopwords): Die Variable beinhaltet eine lange Liste an Wörtern bzw. Stoppwörtern, die zur Filterung angewandt werden sollen.
* Diese Liste an Wörtern wurde von der Datenbasis siehe: http://www.ranks.nl/stopwords entnommen. Es enthält die meisten gebräuchlichen englischen Stoppwörter.
* Hierzu gehören Adjektive, Verben, Präpositionen etc.
*/

function tokenizer(){
	
var stopwords = 'a, able, about, above,abst,accordance,according,accordingly,and,across,act,actually,added,adj,affected, affecting, affects, after, afterwards,\
...
throug,through,throughout,thru,thus,til,tip,to,together,too,took,or,toward,towards,tried,tries,truly,try,trying,ts,twice,two,u,un,under,unfortunately,unless,unlike,unlikely,until,\
...
* stopwords: Hier wird das Objekt (commonObj) mit den übligen Stoppwörtern (stopwords) befüllt.
*/

	function getUncommon(){
		var wordArr = final_transcript.match(/\S+/g),
			commonObj = {},
			word, 
			i;
